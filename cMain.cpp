...
void cMain::generateField(int fx, int fy) {
	/*
		This is the function that is called when the user clicks the first square. It:
		- ensure that current square is not a mine
		- fill board with mineCount # of mines
		TODO:
		- SET ALL ADJACENT NODES TO NOT CONTAIN A MINE
		- first case can be corner, edge, or anywhere else on the board.
	*/
	...
	bool edge = false;
	bool reg = false;
	if ((fx - 1 < 0 && (fy - 1 < 0 || fy + 1  >= fieldHeight)) || (fx + 1 >= fieldWidth && (fy - 1 < 0 || fy + 1 >= fieldHeight)) {
		//First click is a corner value
		corner = true;
	}
	else if (fx - 1 < 0 || fy - 1 < 0 || fx + 1 >= fieldWidth || fy + 1 >= fieldHeight) {
		//First click is a non-corner edge value
		edge = true;
	}
	else {
		//First click is a non-edge non-corner value
	}
	while (mines) {
		/*
		...
		*/
		...
		if (field[ry * fieldWidth + rx] == 0 && rx != fx && ry != fy) {
